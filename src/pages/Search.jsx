...
function Search() {
  const { data, setData, location, user } = useGlobalContext();
  ...
  const searchQueryTitle = new URLSearchParams(location.search).get("title");
  ...
  const getSearch = () => {
    //home
    if (pathname === "home") {
      if (searchQueryTitle !== null && searchQueryTitle !== "") {
        const arrayKeys = [searchQueryTitle];
        const searchResult = data.filter((item) => {
          return arrayKeys.some((key) =>
            item.title.toLowerCase().includes(key.toLocaleLowerCase())
          );
        });
    ...
    if (pathname === "movies") {
      if (searchQueryTitle !== null && searchQueryTitle !== "") {
        const arrayKeys = [searchQueryTitle];
        const arrayMovies = data.filter((item) => {
          return item.category === "Movie";
        });
        const searchResult = arrayMovies.filter((item) => {
          return arrayKeys.some((key) =>
            item.title.toLowerCase().includes(key.toLocaleLowerCase())
          );
        });
    ...
    //tvseries
    if (pathname === "tvseries") {
      if (searchQueryTitle !== null && searchQueryTitle !== "") {
        const arrayKeys = [searchQueryTitle];
        const arrayTVSeries = data.filter((item) => {
          return item.category === "TV Series";
        });
        const searchResult = arrayTVSeries.filter((item) => {
          return arrayKeys.some((key) =>
            item.title.toLowerCase().includes(key.toLocaleLowerCase())
          );
        });
    ...
    //bookmarkç
    if (pathname === "bookmarket") {
      if (searchQueryTitle !== null && searchQueryTitle !== "") {
        const arrayKeys = [searchQueryTitle];
        const arrayBookmarket = data.filter((item) => {
          return item.isBookmarked === true;
        });
        const searchResult = arrayBookmarket.filter((item) => {
          return arrayKeys.some((key) =>
            item.title.toLowerCase().includes(key.toLocaleLowerCase())
          );
        });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    navigate(`../${pathname}/search?title=${search.trim()}`);
  };
  ...
  return (
    <>
      <Helmet>
        <title>Search - Entertainment Web App</title>
      ...
      >
        <div className="max-w-[1440px] mx-auto sm:pt-4 lg:flex lg:py-8 lg:container">
                    ...
                    <figure>
                      ...
                      >
                        <path
                          ...
                          fill={`${pathname == "movies" ? "#fff" : "#5A698F"} `}
                          opacity=".75"
                        />
                      </svg>
                    </figure>
                    ...
                    <figure>
                      ...
                      >
                        <path
                          ...
                          fill={`${pathname == "tvseries" ? "#fff" : "#5A698F"} `}
                          opacity=".75"
                        />
                      </svg>
                    </figure>
                    ...
                    <figure>
                      ...
                      >
                        <path
                          ...
                          fill={`${pathname == "bookmarket" ? "#fff" : "#5A698F"} `}
                        />
                      </svg>
                    </figure>
                  </Link>
          ...
          <main className="lg:container lg:w-full">
            <div className="sm:container mx-auto p-4 smxl:container">
              {/* form */}
              <div>
                <form onSubmit={handleSubmit} className="flex w-full font-light text-white">
                  <div className="w-full flex items-center gap-2">
                    ...
                    <input
                      ...
                      type="text"
                      placeholder={`${pathname == "home" ? "Search for movies or TV series" : ""} ${
                        pathname == "tvseries" ? "Search for TV series" : ""
                      } ${pathname == "bookmarket" ? "Search for bookmarked shows" : ""} ${
              ...
              {/* search text result */}
              {searchQueryTitle !== undefined && searchQueryTitle !== "" && (
                <p className="text-white font-light text-[20px] mb-8">
                  {`Found ${searchResults.length} results for '${
                    searchQueryTitle == null ? "" : searchQueryTitle
                  }'`}
                </p>
              )}
              <div className="grid gap-x-4 gap-y-8 grid-cols-1 sm:grid-cols-2 smxl:grid-cols-3 md:grid-cols-4">
                {/* items cards recommended */}
                {searchQueryTitle !== undefined &&
                  searchQueryTitle !== "" &&
                  data.length > 0 &&
