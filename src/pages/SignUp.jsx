  ...
  const handleFormSubmit = (e) => {
    ...
    if (form.password === form.repeat_password) {
      register({ email: form.email, password: form.password })
        .then((user) => {
          localStorage.setItem(user.user.uid, JSON.stringify(dataJSON));
        })
        .catch((error) => {
        ...
        >
          <div className="flex flex-col gap-8">
            ...
            <div className="relative">
              ...
              />
              {/* show error */}
              {form.repeat_password.trim() === "" && submitted && (
                <div className="absolute right-2 top-2  text-custom_red">Can't be empty</div>
              )}
            </div>
            <div className="relative">
